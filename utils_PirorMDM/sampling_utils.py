...
def double_take_arb_len(args, diffusion, model, model_kwargs, n_frames, eval_mode=False):
    ...
    if args.double_take:
        ...
        old_guidacnce_param = args.guidance_param       # 2.5
        ...
        new_sample_seq_len = (sample.shape[-1] - 2 * handshake_size) * 2 + handshake_size       # (123-20*2)*2+20 = 186

        bs, feats, joints, seq_len = sample.shape       # [4, 263, 1, 123]
        new_sample = torch.zeros((bs-1, feats, joints, new_sample_seq_len), dtype=sample.dtype, device=sample.device)       # [3, 263, 1, 186]
