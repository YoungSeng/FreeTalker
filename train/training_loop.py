    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # for motion, cond in self.data:
                ...
                motion = motion.to(self.device)     # [384, 168, 1, 190]
                cond['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond['y'].items()}       # ['mask', 'lengths', 'text', 'tokens']
                cond['y']['seed'] = motion[..., :self.n_seed]
                cond['y']['audio'] = cond['y']['audio'][:, self.n_seed:]
                ...
                if self.step % 100000 == 0:       # self.save_interval
                    self.save()
                    # self.model.eval()
                    # self.evaluate()
                    # self.model.train()
