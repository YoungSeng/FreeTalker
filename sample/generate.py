...
def main():
    ...
    for rep_i in range(args.num_repetitions):
        ...
        sample = sample_fn(
            ...
            skip_timesteps=990,  # 0 is the default value - i.e. don't skip any step
            init_image=None,
            progress=True,
            dump_steps=None,
            noise=None,
            const_noise=False,
        )       # [1, 263, 1, 196] [1, 22, 3, len]

        # Recover XYZ *positions* from HumanML3D vector representation
        if model.data_rep == 'hml_vec':
            n_joints = 22 if sample.shape[1] == 263 else 21
            sample = data.dataset.t2m_dataset.inv_transform(sample.cpu().permute(0, 2, 3, 1)).float()       # [1, 1, 196, 263], * std + mean
            sample = recover_from_ric(sample, n_joints)     # [1, 1, 196, 22, 3] !!!!!!!!!
            sample = sample.view(-1, *sample.shape[2:]).permute(0, 2, 3, 1)     # [1, 22, 3, 196]

        rot2xyz_pose_rep = 'xyz' if model.data_rep in ['xyz', 'hml_vec'] else model.data_rep            # 'xyz'
