...
def plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
                   vis_mode='default', gt_frames=[], handshake_size=0, blend_size=0, step_sizes=[], lengths=[],
                   joints2=None, painting_features=[]):
    matplotlib.use('Agg')
    """
    ...
    if vis_mode == 'unfold':
        frame_colors = ['purple'] * handshake_size + ['blue'] * blend_size + ['orange'] * (
                    120 - handshake_size * 2 - blend_size * 2) + ['orange'] * blend_size
        ...
        for ii, step_size in enumerate(step_sizes):
            if ii == 0:
                frame_colors = ['orange'] * (step_size - handshake_size - blend_size) + ['orange'] * blend_size + [
                    'purple'] * (handshake_size // 2)
                continue
            if ii == len(step_sizes) - 1:
                frame_colors += ['purple'] * (handshake_size // 2) + ['orange'] * blend_size + ['orange'] * (
                            lengths[ii] - handshake_size - blend_size)
                continue
            frame_colors += ['purple'] * (handshake_size // 2) + ['orange'] * blend_size + ['orange'] * (
    ...
    explicit_plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=figsize, fps=fps, radius=radius,
                            vis_mode=vis_mode, frame_colors=frame_colors, joints2=joints2,
                            painting_features=painting_features)


def explicit_plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
                            vis_mode="default", frame_colors=[], joints2=None, painting_features=[]):
    """
    ...
    def plot_yzPlane(miny, maxy, minz, maxz, minx):
        ## Plot a plane YZ
        verts = [[minx, miny, minz], [minx, miny, maxz], [minx, maxy, maxz], [minx, maxy, minz]]
        yz_plane = Poly3DCollection([verts])
        yz_plane.set_facecolor((0.5, 0.5, 0.5, 0.5))
    ...
    if dataset == "humanml":
        data *= 1.1  # scale for visualization
    ...
    colors_dict = {"blue": colors_blue, "orange": colors_orange, "purple": colors_purple,
    ...
    # trajec = data[:, 0].copy()      # trajec is the trajectory of the root joint
    ...
    def update(index):
        ...
        # plot_xzPlane(MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], 0, MINS[2] - trajec[index, 2],
        #              MAXS[2] - trajec[index, 2])
        plot_xzPlane(MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], 0, MINS[2] - trajec[index, 1],
                     MAXS[2] - trajec[index, 1])

        # plot_xyPlane(MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], 0, MINS[1], MAXS[1])
        # plot_yzPlane(MINS[1], MAXS[1], MINS[2] - trajec[index, 2], MAXS[2] - trajec[index, 2], 0)
        ...
        def plot_root_horizontal():
            ax.plot3D(trajec[:index, 0] - trajec[index, 0], np.zeros_like(trajec[:index, 1]),
                      trajec[:index, 2] - trajec[index, 2], linewidth=2.0,
                      color=used_colors[0])

        def plot_root():
            ax.plot3D(trajec[:index, 0] - trajec[index, 0], trajec[:index, 1], trajec[:index, 2] - trajec[index, 2],
                      linewidth=2.0,
                      color=used_colors[0])
...
def explicit_plot_3d_motion(save_path, kinematic_tree, joints, title, dataset, figsize=(3, 3), fps=120, radius=3,
    ...
    def update(index):
        ...
        # plot_xzPlane(MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], 0, MINS[2] - trajec[index, 2],
        #              MAXS[2] - trajec[index, 2])
        plot_xyPlane(MINS[0] - trajec[index, 0], MAXS[0] - trajec[index, 0], MINS[1], MAXS[1],
                     MINS[2] - trajec[index, 2])

        used_colors = colors_dict[frame_colors[index]] if (index < len(frame_colors)) else colors_dict["blue"]
        other_colors = used_colors  # colors_purple
        for i, (chain, color, other_color) in enumerate(zip(kinematic_tree, used_colors, other_colors)):
            if i < 5:
                linewidth = 3.25 / 1     # 20230911
            else:
                linewidth = 1.75 / 1     # 2 -> 1.5
